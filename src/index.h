...
struct KnnQueryResult {
    ...
    std::size_t admitted_subgraphs{0};
    std::size_t admitted_objects{0};
    std::size_t candidate_subgraphs{0};
    ...
    bool subgraph_admit{false};
    bool subgraph_admit_auto_disabled{false};
    double object_density{0.0};
    std::size_t occupied_subgraphs{0};
    double occupied_subgraph_ratio{0.0};
    double avg_objects_per_occupied_subgraph{0.0};
    std::size_t max_objects_in_subgraph{0};
    long long init_us{0};
    ...
    std::size_t factorized_hubs_used{0};
    std::size_t factorized_exits_emitted{0};
    EdgeWeight final_kth_exact{kInfWeight};
};
...
struct InvertedSubgraphBoundaryEntry {
    SgId subgraph_id{0};
    std::uint32_t clique_row_id{0};
};
...
public:
    ...
    void release_subgraph_distances();
    ...
    [[nodiscard]] const std::vector<std::unordered_map<VertexId, std::vector<std::pair<VertexId, EdgeWeight>>>>&
    subgraph_clique_rows() const;
    RangeQueryResult range_query(
        ...
        bool allow_whole_subgraph_acceptance = true
    ) const;
    ...
    KnnQueryResult knn_query(
        ...
        bool use_factorized_transfer = false,
        bool knn_direct_row_path = false,
        bool subgraph_admit = false,
        bool subgraph_admit_exact_cap = false,
        bool subgraph_admit_density_fallback = false
    ) const;
...
private:
    ...
    void initialize_runtime_scratch();
    void rebuild_cached_runtime_views();
    ...
    std::vector<std::vector<InvertedSubgraphBoundaryEntry>> inverted_boundary_index_fast_;
    ...
    std::vector<std::vector<std::vector<std::pair<VertexId, EdgeWeight>>>> subgraph_clique_row_storage_;
    ...
    mutable std::vector<std::uint32_t> knn_admitted_subgraph_stamp_;
