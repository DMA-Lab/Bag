...
RangeQueryResult SkeletonIndex::range_query(
    ...
    std::vector<EdgeWeight> pc_border_dists;
    for (const auto sg_id : hit_subgraphs) {
        ...
        ++result.pc_subgraphs;
        if (fast_dense_path) {
            // Evaluate objects directly against their border-cost rows: an object
            // o is in range iff min_b (d(b) + cost(b,o)) <= radius. Extract the
            // subgraph's in-range border distances once (contiguous, by local
            // index), then scan objects in suffix-ascending order — the first
            // object whose suffix exceeds radius - min_border_dist puts every
            // later object out of range, so the loop stops early. This replaces
            // the per-subgraph local Dijkstra with a sequential SoA scan.
            const auto& order = objects.objects_in_knn_order(sg_id);
            if (order.empty()) {
                continue;
            }
            const auto& boundaries = subgraph_boundaries_[sg_id];
            pc_border_dists.assign(boundaries.size(), kInfWeight);
            EdgeWeight sg_lb = kInfWeight;
            for (std::uint32_t li = 0; li < boundaries.size(); ++li) {
                const auto b = boundaries[li];
                if (b < skeleton_scratch_stamp_.size() && skeleton_scratch_stamp_[b] == fast_boundary_epoch) {
                    const auto d = skeleton_scratch_dist_[b];
                    if (d <= radius) {
                        pc_border_dists[li] = d;
                        sg_lb = std::min(sg_lb, d);
                    }
                }
            }
            if (sg_lb == kInfWeight) {
            ...
            const auto cost_budget = static_cast<EdgeWeight>(radius - sg_lb);
            const auto& suffixes = objects.knn_order_suffixes(sg_id);
            for (std::size_t j = 0; j < order.size(); ++j) {
                if (suffixes[j] > cost_budget) {
                    break;  // suffix ascending: all remaining objects are out of range
                }
                ++result.exact_checked_objects;
                bool within = false;
                for (const auto& [local_idx, cost] : objects.knn_order_costs(sg_id, j)) {
                    if (cost > cost_budget) {
                        break;
                    }
                    const auto d = pc_border_dists[local_idx];
                    if (d != kInfWeight && cost <= static_cast<EdgeWeight>(radius - d)) {
                        within = true;
                        break;
                    }
                }
                if (within && mark_seen_object(order[j])) {
                    result.object_ids.push_back(order[j]);
                    ++result.exact_returned_objects;
                }
            }
            continue;
