...
int main(int argc, char** argv) {
    try {
        ...
        // Border minimization is enabled by default in this build: it demotes
        // conservatively-marked border vertices back to internal (whenever they
        // have no external edge and the BR-property still holds), shrinking the
        // skeleton ~26% at no extra build cost and speeding queries ~15%. Pass
        // --border-min false to reproduce the paper's un-minimized partition.
        const bool border_min = optional_bool(args, "border-min", true);
